...
void UIWeatherPropForm::Draw()
{
	...
	if (ImGui::BeginChild("Center", ImVec2(455, 575), true))
	{
		...
		// Выбор погодного цикла
		{
			...
			// --------------------------------------------------------------------------------------------
			if (ImGui::BeginCombo("Weather cycle", selectedWeather ? env.CurrentCycleName.c_str() : "None"))
			{
				bool SelectedNone = !psDeviceFlags.test(rsEnvironment);
				...
				for (const auto& [Name, _] : env.WeatherCycles)
				{
					selectedWeather = psDeviceFlags.test(rsEnvironment) && Name == EPrefs->sWeather;
					if (ImGui::MenuItem(Name.c_str(), "", &selectedWeather))
					{
...
{
	bIsShowSun = m_sun_visible && !bIsShowSun;
}

void UIWeatherPropForm::IsUseSunDirChanged() const
{
	bIsUseSunDir = m_use_sun_dir && !bIsUseSunDir;
}

void UIWeatherPropForm::IsUseHemiChanged() const
{
	bIsUseHemi = m_use_hemi && !bIsUseHemi;
}

void UIWeatherPropForm::IsRaindropCollisionChanged() const
{
	bIsRaindropCollision = m_raindrop_collision && !bIsRaindropCollision;
}

void UIWeatherPropForm::IsSndOnRoofChanged() const
{
	bIsSndOnRoof = m_snd_on_roof && !bIsSndOnRoof;
}
