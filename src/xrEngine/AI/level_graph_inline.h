...
IC	void ILevelGraph::unpack_xz(const ILevelGraph::CPosition &vertex_position, int &x, int &z) const
{
	u32					_x, _z;
...
IC	void ILevelGraph::unpack_xz(const ILevelGraph::CPosition &vertex_position, float &x, float &z) const
{
	u32					_x,_z;
	unpack_xz			(vertex_position,_x,_z);
...
template <typename T>
IC	void ILevelGraph::unpack_xz(const ILevelGraph::CVertex &vertex, T &x, T &z) const
{
...
ICF	const Fvector ILevelGraph::vertex_position	(const ILevelGraph::CPosition &source_position) const
{
...
ICF	const Fvector &ILevelGraph::vertex_position	(Fvector &dest_position, const ILevelGraph::CPosition &source_position) const
{
	return				(dest_position = vertex_position(source_position));
}

IC	const ILevelGraph::CPosition &ILevelGraph::vertex_position	(ILevelGraph::CPosition &dest_position, const Fvector &source_position) const
{
...
IC	const ILevelGraph::CPosition	ILevelGraph::vertex_position	(const Fvector &position) const
{
	ILevelGraph::CPosition	_vertex_position;
	return				(vertex_position(_vertex_position,position));
}

IC bool	ILevelGraph::inside				(const ILevelGraph::CVertex &vertex, const ILevelGraph::CPosition &vertex_position) const
{
	return 				(vertex_position.xz() == vertex.position().xz());
}

IC bool	ILevelGraph::inside				(const ILevelGraph::CVertex &vertex, const Fvector &position) const
{
...
IC bool	ILevelGraph::inside				(const CVertex *vertex, const ILevelGraph::CPosition &vertex_position) const
{
	return				(inside(*vertex,vertex_position));
}

IC bool ILevelGraph::inside				(const CVertex *vertex, const Fvector &position) const
{
	return				(inside(*vertex,position));
}

IC bool	ILevelGraph::inside				(const u32 vertex_id, const ILevelGraph::CPosition &vertex_position) const
{
	return				(inside(vertex(vertex_id),vertex_position));
}

IC bool ILevelGraph::inside				(const u32 vertex_id, const Fvector &position) const
{
...
IC bool	ILevelGraph::inside				(const CVertex *vertex, const ILevelGraph::CPosition &vertex_position, const float epsilon) const
{
	return				(inside(*vertex,vertex_position,epsilon));
}

IC bool ILevelGraph::inside				(const CVertex *vertex, const Fvector &position, const float epsilon) const
{
	return				(inside(*vertex,position,epsilon));
}

IC bool	ILevelGraph::inside				(const u32 vertex_id, const ILevelGraph::CPosition &vertex_position, const float epsilon) const
{
	return				(inside(vertex(vertex_id),vertex_position,epsilon));
}

IC bool ILevelGraph::inside				(const u32 vertex_id, const Fvector &position, const float epsilon) const
{
...
IC bool	ILevelGraph::inside				(const u32 vertex_id,	const Fvector2 &position) const
{
	...
	{
		Msg("! Invalid vertex id %u at pos xz[%.3f %.3f]", vertex_id, position.x, position.y);
		return false;
	}
...
IC float ILevelGraph::vertex_plane_y	(const ILevelGraph::CVertex &vertex, const float X, const float Z) const
{
...
IC float ILevelGraph::vertex_plane_y	(const ILevelGraph::CVertex &vertex) const
{
	float				x,z;
	unpack_xz			(vertex,x,z);
...
IC float ILevelGraph::vertex_plane_y	(const u32 vertex_id, const float X, const float Z) const
{
	return				(vertex_plane_y(vertex(vertex_id),X,Z));
}

IC float ILevelGraph::vertex_plane_y	(const u32 vertex_id) const
{
	return				(vertex_plane_y(vertex(vertex_id)));
}

ICF u32	ILevelGraph::CHeader::version() const
{
	return				(hdrNODES::version);
}

ICF u32	ILevelGraph::CHeader::vertex_count() const
{
	return				(count);
}

ICF float	ILevelGraph::CHeader::cell_size() const
{
	return				(size);
}

ICF float	ILevelGraph::CHeader::factor_y() const
{
	return				(size_y);
}

ICF const Fbox &ILevelGraph::CHeader::box() const
{
	return				(aabb);
}

ICF const xrGUID &ILevelGraph::CHeader::guid() const
{
...
IC	const GameGraph::_LEVEL_ID &ILevelGraph::level_id() const
{
	return				(m_level_id);
}

IC	void ILevelGraph::level_id(const GameGraph::_LEVEL_ID &level_id)
{
	m_level_id			= level_id;
}

IC	void ILevelGraph::begin(const CVertex &vertex, const_iterator &begin, const_iterator &end) const
{
	begin				= 0;
	end					= 4;
}

IC	void ILevelGraph::begin(const CVertex *_vertex, const_iterator &_begin, const_iterator &end) const
{
	begin				(*_vertex,_begin,end);
}

IC	void ILevelGraph::begin(const u32 vertex_id, const_iterator &_begin, const_iterator &end) const
{
	begin				(vertex(vertex_id),_begin,end);
}

IC	u32	ILevelGraph::value(const CVertex &vertex, const_iterator &i) const
{
...
IC	u32	ILevelGraph::value(const u32 vertex_id, const_iterator &i) const
{
	return				(value(vertex(vertex_id),i));
}

IC	bool ILevelGraph::is_accessible		(const u32 vertex_id) const
{
	return				(valid_vertex_id(vertex_id) && m_access_mask[vertex_id]);
}

IC	void ILevelGraph::set_invalid_vertex(u32 &vertex_id, CVertex **vertex) const
{
	vertex_id			= u32(-1);
	VERIFY				(!valid_vertex_id(vertex_id));
	if (vertex)
...
IC  Fvector2 ILevelGraph::v2d(const Fvector &vector3d) const
{
	return				(Fvector2().set(vector3d.x,vector3d.z));
}

template <bool bAssignY, typename T>
IC	bool	ILevelGraph::create_straight_path	(u32 start_vertex_id, const Fvector2 &start_point, const Fvector2 &finish_point, xr_vector<T> &tpaOutputPoints, const T &example, bool bAddFirstPoint, bool bClearPath) const
{
		...
		for ( ; I != E; ++I) {
			...
			if (box.pick_exact(start,dir)) {
				...
				if (tpaOutputPoints.size() > 100000) {
					Msg			(
						"ILevelGraph::create_straight_path : Loop became infinite (%d,[%f][%f][%f],[%f][%f][%f])",
						start_vertex_id,
...
template<typename T>
IC	void ILevelGraph::assign_y_values		(xr_vector<T> &path)
{
...
IC	u32	ILevelGraph::row_length				() const
{
	return						(m_row_length);
}

IC	bool ILevelGraph::valid_vertex_position	(const Fvector &position) const
{
...
IC	void ILevelGraph::set_mask				(const xr_vector<u32> &mask)
{
...
IC	void ILevelGraph::set_mask_no_check		(const xr_vector<u32> &mask)
{
	xr_vector<u32>::const_iterator	I = mask.begin();
	xr_vector<u32>::const_iterator	E = mask.end();
	for ( ; I != E; ++I)
...
IC	void ILevelGraph::clear_mask			(const xr_vector<u32> &mask)
{
	xr_vector<u32>::const_iterator	I = mask.begin();
	xr_vector<u32>::const_iterator	E = mask.end();
	for ( ; I != E; ++I)
...
IC	void ILevelGraph::clear_mask_no_check	(const xr_vector<u32> &mask)
{
	xr_vector<u32>::const_iterator	I = mask.begin();
	xr_vector<u32>::const_iterator	E = mask.end();
	for ( ; I != E; ++I)
...
IC	void ILevelGraph::set_mask				(u32 vertex_id)
{
	VERIFY						(m_access_mask[vertex_id]);
	set_mask_no_check			(vertex_id);
}

IC	void ILevelGraph::set_mask_no_check		(u32 vertex_id)
{
	m_access_mask[vertex_id]	= false;
}

IC	void ILevelGraph::clear_mask			(u32 vertex_id)
{
	VERIFY						(!m_access_mask[vertex_id]);
	clear_mask_no_check			(vertex_id);
}

IC	void ILevelGraph::clear_mask_no_check	(u32 vertex_id)
{
...
IC	u32	ILevelGraph::max_x					() const
{
	return						(m_max_x);
}

IC	u32	ILevelGraph::max_z					() const
{
	return						(m_max_z);
}

IC	bool operator<		(const ILevelGraph::CVertex &vertex, const u32 &vertex_xz)
{
...
IC	bool operator<		(const u32 &vertex_xz, const ILevelGraph::CVertex &vertex)
{
