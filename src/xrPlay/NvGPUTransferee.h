...
// The NVAPI SDK header IS bundled (sdk/include/nvapi/, see "Add GPU SDK
// files"); the original `#include <NVAPI\nvapi.h>` only worked on Windows
// because its path lookup is case-insensitive. The on-disk directory is
// lowercase, so spell it out for case-sensitive hosts.
#include <nvapi/nvapi.h>

#define NVAPI_MAX_USAGES_PER_GPU 34
class CNvReader : public ICore_GPU {
  ...
  using NvAPI_PhysicalFromLogical = int (*)(int *handle1, int **handle,
                                            unsigned long *count);
