...
CLocatorAPI::~CLocatorAPI() { VERIFY(0 == m_iLockRescan); }

void CLocatorAPI::Register(LPCSTR name, u32 vfs, u32 crc, u32 ptr,
                           u32 size_real, u32 size_compressed, time_t modif) {
...
IReader *open_chunk(FileHandle ptr, u32 ID, pcstr archiveName, u32 archiveSize,
                    bool shouldDecrypt = false) {
  u32 dwType;
  u32 dwSize = 0;
  size_t read_byte;

  if (Platform::SetFilePointer(ptr, 0, FILE_BEGIN) ==
  ...
  while (true) {
    ...
    if ((dwType & (~CFS_CompressMark)) == ID) {
      ...
      if (Platform::SetFilePointer(ptr, dwSize, FILE_CURRENT) ==
          INVALID_SET_FILE_POINTER) {
        return nullptr;
      }
    }
  ...
  if (A.header) {
    ...
    if (0 == _stricmp(read_path.c_str(), "gamedata")) {
      ...
      if (P != pathes.end()) {
        ...
        //				R_ASSERT3
        //(root, "path not found ", read_path.c_str());
...
#ifdef IXR_WINDOWS
  HANDLE h = CreateFile(
      Platform::ANSI_TO_TCHAR_U8(_path), 0, 0, nullptr, OPEN_EXISTING,
...
bool CLocatorAPI::Recurse(const char *path) {
  ...
  for (const xr_dir_entry &CurrentFile : xr_dir_iter{N}) {
...
#ifdef IXR_WINDOWS
    xr_string ValidFileName =
        Platform::TCHAR_TO_ANSI_U8(currentPath.generic_wstring().c_str());
#else
    xr_string ValidFileName =
        Platform::TCHAR_TO_ANSI_U8(currentPath.generic_string().c_str());
#endif
    if (bWrapPath)
    ...
    if (StatusFile.type() == std::filesystem::file_type::not_found ||
...
#ifdef IXR_WINDOWS
    if (GetFileAttributes(currentPath.generic_wstring().c_str()) &
...
void *FileDownload(LPCSTR file_name, const int &file_handle,
                   intptr_t &file_size);
...
void CLocatorAPI::setup_fs_path(LPCSTR fs_name) {
  ...
  // Probe the *effective* fs-ltx name: a custom -fsltx path when given,
  // otherwise the default FSLTX.
  xr_string probe_name = (fs_name && fs_name[0]) ? fs_name : FSLTX;
  if (!std::filesystem::exists(TestPath + "/" + probe_name)) {
    auto TryTestPath = [&TestPath, &probe_name](auto Path) {
      xr_path TryPath = Path;
      xr_string StrPath = TryPath.parent_path().generic_string().c_str();

      if (std::filesystem::exists(StrPath + "/" + probe_name)) {
...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder, LPCSTR fs_name) {
  ...
  if (m_Flags.is(flTargetFolderOnly)) {
    ...
    while (!pFSltx->eof()) {
      ...
      FS_Path *P =
          new FS_Path((p_it != pathes.end()) ? p_it->second->m_Path : root,
      ...
      CHECK_OR_EXIT(
          I.second,
          "The file 'fsgame.ltx' is corrupted (it contains duplicated "
  ...
  if (FS.path_exist("$arch_dir_addons$")) {
    ...
    FS.rescan_path(AddonsArchsPath->m_Path,
                   AddonsArchsPath->m_Flags.is(FS_Path::flRecurse));
    FS.IsAddonPhase = false;
  }

  // u32	M2 = Memory.mem_usage();
  // Msg("FS: %d files cached %d archives, %dKb memory used.", m_files.size(),
...
const CLocatorAPI::file *CLocatorAPI::exist(string_path &fn, LPCSTR path,
                                            LPCSTR name) {
...
int CLocatorAPI::file_list(FS_FileSet &dest, LPCSTR path, u32 flags,
                           LPCSTR mask) {
  R_ASSERT(path);
  VERIFY(flags);
  // проверить нужно ли пересканировать пути
...
void CLocatorAPI::check_cached_files(LPSTR fname, const u32 &fname_size,
                                     const file &desc, LPCSTR &source_name) {
  ...
  if (bCopy) {
    ...
    Register(fname_in_cache, 0xffffffff, 0, 0, desc.size_real, desc.size_real,
             desc.modif);
  }
...
void CLocatorAPI::file_from_cache_impl(IReader *&R, LPSTR fname,
                                       const file &desc) {
...
void CLocatorAPI::file_from_cache_impl(CStreamReader *&R, LPSTR fname,
                                       const file &desc) {
  const char *RealFileName = desc.wrap == nullptr ? fname : desc.wrap;
...
template <typename T>
void CLocatorAPI::file_from_cache(T *&R, LPSTR fname, const u32 &fname_size,
                                  const file &desc, LPCSTR &source_name) {
#ifdef DEBUG
...
void CLocatorAPI::file_from_archive(IReader *&R, LPCSTR fname,
                                    const file &desc) {
  // Archived one
...
void CLocatorAPI::file_from_archive(CStreamReader *&R, LPCSTR fname,
                                    const file &desc) {
  archive &A = m_archives[desc.vfs];
  R_ASSERT2(
      desc.size_compressed == desc.size_real,
      make_string<const char *>("cannot use stream reading for compressed data "
                                "%s, do not compress data to be streamed",
                                fname));
  ...
  R->construct(
      ...
      A.hSrcFile,
#endif
      desc.ptr, desc.size_compressed, A.size, BIG_FILE_READER_WINDOW_SIZE);
}
...
void CLocatorAPI::r_close(IReader *&fs) { xr_delete(fs); }

void CLocatorAPI::r_close(CStreamReader *&fs) { fs->close(); }

void CLocatorAPI::get_all_files_in_dir(xr_set<xr_string> &out, LPCSTR dir) {
  for (auto &elem : m_files) {
    if (xr_strlen(elem.name) > xr_strlen(dir) &&
        std::isalpha(elem.name[xr_strlen(elem.name) - 1]) &&
        !xr_strncmp(elem.name, dir, xr_strlen(dir) - 1)) {
...
bool CLocatorAPI::TryLoad(const xr_string &File) {
  ...
  if (!Found) {
    ...
    if (Found) {
      ...
      size_t FileModif =
          xr_chrono_to_time_t(std::filesystem::last_write_time(File.c_str()));
      FS.Register(File.c_str(), 0xffffffff, 0, 0, FileSize, FileSize,
                  FileModif);
    }
  }
...
FS_Path *CLocatorAPI::append_path(LPCSTR path_alias, LPCSTR root, LPCSTR add,
                                  BOOL recursive) {
  VERIFY(root);
