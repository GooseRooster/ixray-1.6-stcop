...
SvGraphNodeReference Texmaps::SvTraverseAnimGraph(IGraphObjectManager *gom, Animatable *owner, int id, DWORD flags)
	...
	//if (!gom->TestFilter(SV_FILTER_MAP))
...
TSTR Texmaps::SubAnimName(int i, bool) {
	if (i&1)
...
RefResult Texmaps::NotifyRefChanged(
...
RefTargetHandle Texmaps::Clone(const RemapDir &remap) 
{
	RemapDir* pRemap = (RemapDir*) & remap;
	...
	for (int i = 0; i<STD2_NMAX_TEXMAPS; i++) {
		...
		tm->txmap[i].map = NULL;
		if (txmap[i].amtCtrl) 
			tm->ReplaceReference(2*i, pRemap->CloneRef(txmap[i].amtCtrl));
		if (txmap[i].map) 
			tm->ReplaceReference(2*i+1, pRemap->CloneRef(txmap[i].map));
		}
	BaseClone(this, tm, *pRemap);
	return tm;
	}
