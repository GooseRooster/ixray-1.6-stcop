...
static ParamBlockDescID extVer10[] = {
	...
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// bounce
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// static friction
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// sliding friction
...
static ParamBlockDescID dynVer10[] = {
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// opacity
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// opfalloff
	{ TYPE_RGBA,  NULL, TRUE, DWORD(-1) },					// filter
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// wireSize
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// index of refraction
	...
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// reflect dim level
	{ TYPE_FLOAT, NULL, TRUE, DWORD(-1) },					// reflect dim multiplier 
};

// v10 Param Block Descriptor
static ParamBlockDescID stdmtl2PB[NPARAMS] = {
	{ TYPE_FLOAT, NULL, TRUE, 7u },   // opacity
	{ TYPE_FLOAT, NULL, TRUE, 8u }, 	// opfalloff
	{ TYPE_RGBA,  NULL, TRUE, 9u },   // filter
	{ TYPE_FLOAT, NULL, TRUE, 10u },  // wireSize
	{ TYPE_FLOAT, NULL, TRUE, 11u },  // index of refraction
	{ TYPE_FLOAT, NULL, TRUE, 12u },  // bounce
	{ TYPE_FLOAT, NULL, TRUE, 13u },  // static friction
	{ TYPE_FLOAT, NULL, TRUE, 14u },  // sliding friction
	{ TYPE_FLOAT, NULL, TRUE, 15u },  // reflect dim level
	{ TYPE_FLOAT, NULL, TRUE, 16u },  // reflect dim multiplier 
};
...
static ParamBlockDescID stdmtlPB[NPARAMS_O] = {
	{ TYPE_RGBA, NULL, TRUE, 1u },    // ambient
	{ TYPE_RGBA, NULL, TRUE, 2u },    // diffuse
	{ TYPE_RGBA, NULL, TRUE, 3u },    // specular
	{ TYPE_FLOAT, NULL, TRUE, 4u },   // shininess
	{ TYPE_FLOAT, NULL, TRUE, 5u },   // shini_strength
	{ TYPE_FLOAT, NULL, TRUE, 6u },   // self-illum
	{ TYPE_FLOAT, NULL, TRUE, 7u },   // opacity
	{ TYPE_FLOAT, NULL, TRUE, 8u }, 	// opfalloff
	{ TYPE_RGBA,  NULL, TRUE, 9u },   // filter
	{ TYPE_FLOAT, NULL, TRUE, 10u },  // wireSize
	...
	{ TYPE_FLOAT, NULL, TRUE, 15u },  // reflect dim level
	{ TYPE_FLOAT, NULL, TRUE, 16u },  // reflect dim multiplier 
	{ TYPE_FLOAT, NULL, TRUE, 17u }   // soften
};
...
void XRayMtl::Reset()
{
	...
	auto a = Class_ID(DEFAULT_SHADER_CLASS_ID, 0);
	SetShaderIndx(FindShader(a));
	...
	auto b = Class_ID(DEFAULT_SHADER_CLASS_ID, 0);
	SetSamplerIndx(FindSampler(b));
...
RefTargetHandle XRayMtl::Clone(const RemapDir& cremap)
{
	RemapDir& remap = (RemapDir&)cremap;
	macroRecorder->Disable();
...
void XRayMtl::SetShading(int s)
{
	...
	auto ClassID = Class_ID(cid, 0);
	long indx = FindShader(ClassID);
...
public:
	...
	RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
		PartID& partID, RefMessage message, BOOL) {
		if (message == REFMSG_TARGET_DELETED) {
...
void XRayMtl::ShuffleTexMaps(Shader* newShader, Shader* oldShader)
{
	...
	// now do the mtl maps
	for (int oldChan = 0, newChan = 0, n = 0; n < MTL_NTEXMAPS; ++n) {
...
void XRayMtl::ShuffleShaderParams(Shader* newShader, Shader* oldShader)
{
	...
	ClassDesc* newCD;
	auto OldID = newShader->ClassID();
	FindShader(OldID, &newCD);

	ClassDesc* oldCD;
	auto aOldID = oldShader->ClassID();
	FindShader(aOldID, &oldCD);
...
TSTR XRayMtl::SubAnimName(int i, bool) {
	switch (i) {
...
TSTR XRayMtl::GetSubTexmapSlotName(int i, bool) {
	if (pShader) {
...
/////////////////////////////////////////////////////////////////////
// shaders
void XRayMtl::SetShaderIndx(int indx, BOOL update)
{
...
void XRayMtl::SetSamplerIndx(int indx, BOOL update)
{
...
void XRayMtl::OldVerFix(int loadVer)
{
	...
	if (loadVer < 10) {
		...
		auto i = Class_ID(cid, 0);
		long indx = FindShader(i);
...
public:
	...
	void proc(ILoad* iload)
	{
		...
		auto ID = m->pShader->ClassID();
		m->SetShaderIndx(m->FindShader(ID), FALSE);
...
void XRayMtl::SetupGfxMultiMaps(TimeValue t, Material* mtl, MtlMakerCallback& cb) {
	if (texHandleValid.InInterval(t)) {
		mtl->texture.reserve(numTexHandlesUsed);
...
void XRayMtl::SetEShaderIndx(int indx)
{
	pb_xray->SetValue(std2_eshader_type, 0, indx < 0 ? 0 : indx);
	eshaderId = indx;
}
void XRayMtl::SetCShaderIndx(int indx)
{
	pb_xray->SetValue(std2_cshader_type, 0, indx < 0 ? 0 : indx);
	cshaderId = indx;
}
void XRayMtl::SetGameMtlIndx(int indx)
{
