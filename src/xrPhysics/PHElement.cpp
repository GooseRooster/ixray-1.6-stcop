...
void CPHElement::SetTransform(const Fmatrix &m0, motion_history_state history_state ){
	VERIFY3(_valid(m0), "invalid_form_in_set_transform %s", (PhysicsRefObject()->dump(full_capped)).c_str());
	VERIFY2(valid_pos(m0.c), dbg_valide_pos_string(m0.c, PhysicsRefObject(), "invalid_form_in_set_transform").c_str());
	Fvector mc;
...
void	CPHElement::	applyImpulseTrace		(const Fvector& pos, const Fvector& dir, float val,u16 id)
{
	...
	if(debug_output().ph_dbg_draw_mask().test(phHitApplicationPoints))
	{
		...
		debug_output().DBG_DrawLine(dbg_position, Fvector().add(dbg_position, Fvector().mul(dir, 0.4f)), color_xrgb(255, 0, 255));
...
{
	if(!m_fratures_holder) m_fratures_holder=new CPHFracturesHolder();
