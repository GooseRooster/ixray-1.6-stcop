...
CPhysicsShell*	 P_build_Shell			( IPhysicsShellHolder* obj, bool not_active_state, BONE_P_MAP* bone_map, bool not_set_bone_callbacks )
{
	VERIFY( obj );
...
CPhysicsShell*	P_build_Shell( IPhysicsShellHolder* obj, bool not_active_state, LPCSTR	fixed_bones )
{
	...
	if(fixed_bones){
		...
		for (int i=0 ;i<count; ++i){
			...
			R_ASSERT2(BI_NONE != f_bones.back(), make_string<const char*>("wrong fixed bone [%s] for object [%s] with visual [%s]", 
				fixed_bone, obj->ObjectName(), obj->ObjectNameVisual()));
		}
	}
