...
static class cl_pos_decompress_params : public R_constant_setup {
  virtual void setup(R_constant *C) {
    ...
    RCache.set_c(C, HorzTan, -VertTan, (2.0f * HorzTan) / RCache.get_width(),
                 -(2.0f * VertTan) / RCache.get_height());
  }
...
static class cl_pos_decompress_params2 : public R_constant_setup {
  virtual void setup(R_constant *C) {
    RCache.set_c(C, RCache.get_width(), RCache.get_height(),
                 1.0f / RCache.get_width(), 1.0f / RCache.get_height());
  }
...
// OWA: hemisphere/weather parameters
// x = hemi vibrance, y = hemi contrast, z = wet surface factor, w = reserved
static class cl_hemi_parameters : public R_constant_setup {
  virtual void setup(R_constant *C) {
    CEnvDescriptor &E = *g_pGamePersistent->Environment().CurrentEnv;
    RCache.set_c(C, E.m_fHemiVibrance, E.m_fHemiContrast, E.m_fWetSurfaces,
                 0.0f);
  }
} binder_hemi_parameters;

// OWA: texture contrast strength (Build 3120 style)
static class cl_tex_contrast : public R_constant_setup {
  virtual void setup(R_constant *C) {
...
void CRender::create() {
  ...
  if (!EngineExternal().ShadersOptions.contains(
          xr_string("USE_LEGACY_LIGHT"))) {
    o.deffered_reflecitons = !!ps_r2_ls_flags_ext.test(R4FLAG_SSLR_ON_WORLD);
    o.offscreen_reflecitons =
        !!ps_r2_ls_flags_ext.test(R4FLAG_OFFSCREEN_REFLECTIONS);
  } else {
    o.deffered_reflecitons = o.offscreen_reflecitons = false;
  }

  o.dx11_enable_tessellation =
      RFeatureLevel >= D3D_FEATURE_LEVEL_11_0 &&
  ...
  dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup(
      "pos_decompression_params_hud", &binder_pos_decompress_params_hud);
  dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup(
      "depth_unpack", &binder_depth_unpack);
  dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup(
      "triLOD", &binder_LOD);
  dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup(
  ...
  // BUG-ID: 10646
  {
    ...
    for (it = 0; it < Lights_LastFrame.size(); it++) {
      ...
      try {
        ...
        Msg("! Failed to flush-OCCq on light [%d] %X", it,
            *(u32 *)(&Lights_LastFrame[it]));
      }
    }
...
void CRender::add_StaticWallmark(ref_shader &S, const Fvector &P, float s,
                                 CDB::TRI *T, Fvector *verts,
...
void CRender::add_StaticWallmark(IWallMarkArray *pArray, const Fvector &P,
...
void CRender::add_SkeletonWallmark(const Fmatrix *xf, CKinematics *obj,
                                   ref_shader &sh, const Fvector &start,
                                   const Fvector &dir, float size) {
  Wallmarks->AddSkeletonWallmark(xf, obj, sh, start, dir, size);
}

void CRender::add_SkeletonWallmark(const Fmatrix *xf, IKinematics *obj,
                                   IWallMarkArray *pArray, const Fvector &start,
                                   const Fvector &dir, float size) {
  dxWallMarkArray *pWMA = (dxWallMarkArray *)pArray;
  ref_shader *pShader = pWMA->dxGenerateWallmark();
  if (pShader) {
...
void CRender::rmNear() {
  ...
  D3D_VIEWPORT VP = {0, 0,    (float)T->get_width(), (float)T->get_height(),
                     0, 0.02f};
...
void CRender::rmFar() {
  IRender_Target *T = getTarget();
  D3D_VIEWPORT VP = {
      0, 0, (float)T->get_width(), (float)T->get_height(), 0.99999f, 1.f};
...
template <typename T>
static HRESULT create_shader(LPCSTR const pTarget, DWORD const *buffer,
                             u32 const buffer_size, LPCSTR const file_name,
                             T *&result, bool const disasm) {
  result->sh = ShaderTypeTraits<T>::CreateHWShader(buffer, buffer_size);

  ID3DShaderReflection *pReflection = 0;

  HRESULT const _hr = D3DReflect(buffer, buffer_size, IID_ID3DShaderReflection,
                                 (void **)&pReflection);
...
static HRESULT create_shader(LPCSTR const pTarget, DWORD const *buffer,
  ...
  if (pTarget[0] == 'p') {
    ...
    _result =
        RDevice->CreatePixelShader(buffer, buffer_size, 0, &sps_result->ps);
    ...
    _result =
        RDevice->CreateVertexShader(buffer, buffer_size, 0, &svs_result->vs);
    ...
    if (SUCCEEDED(_result) && pReflection) {
      ...
      // CHK_DX( D3DxxGetInputSignatureBlob(pShaderBuf->GetBufferPointer(),
    ...
    _result =
        RDevice->CreateGeometryShader(buffer, buffer_size, 0, &sgs_result->gs);
    if (!SUCCEEDED(_result)) {
      Msg("! GS: %s", file_name);
  ...
  if (disasm) {
    ...
    string_path dname;
    xr_strconcat(dname, "disasm\\", file_name,
                 ('v' == pTarget[0])   ? ".vs.hlsl"
                 : ('p' == pTarget[0]) ? ".ps.hlsl"
                                       : ".gs.hlsl");
...
HRESULT CRender::shader_compile(LPCSTR name, DWORD const *pSrcData,
  ...
  for (auto &[Name, Value] : m_ShaderOptions) {
    defines[def_it++] = {Name.c_str(), Value.c_str()};
  }
  ...
  sh_name[len] = '0' + char(ps_r2_ls_flags_ext.test(RFLAG_CLOUD_SHADOWS));
  ++len;
  ...
  if (ps_r_sun_shafts > 0) {
    ...
    def_it++;
    sh_name[len] = '0' + static_cast<char>(ps_r_sun_shafts);
    ++len;
  } else {
    sh_name[len] = '0';
    ++len;
  ...
  sh_name[len] = '0' + char(RFeatureLevel == D3D_FEATURE_LEVEL_10_1);
  ++len;
  ...
  sh_name[len] = '0' + char(RFeatureLevel >= D3D_FEATURE_LEVEL_11_0);
  ++len;
  ...
  if (FAILED(_result)) {
    ...
    _result = D3DCompile(pSrcData, SrcDataLen,
                         "", // nullptr, //LPCSTR pFileName,	//
                             // NVPerfHUD bug workaround.
                         defines, &Includer, pFunctionName, pTarget, Flags, 0,
    ...
    if (SUCCEEDED(_result)) {
      if (/*ps_r__common_flags.test(RFLAG_USE_CACHE)*/ 1) {
        ...
        u32 const crc =
            crc32(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize());
        file->w_u32(crc);
      ...
      _result = create_shader(pTarget, (DWORD *)pShaderBuf->GetBufferPointer(),
                              (u32)pShaderBuf->GetBufferSize(), file_name,
                              result, o.disasm);
    } else {
      Msg("! %s", file_name);
