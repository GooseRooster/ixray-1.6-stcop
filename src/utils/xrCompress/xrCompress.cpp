...
bool xrCompressor::testSKIP(LPCSTR path)
{
	...
	{
		if (0==_stricmp(p_ext,".aimap")	)	return true;
		if (0==_stricmp(p_ext,".cform")	)	return true;
		if (0==_stricmp(p_ext,".details"))	return true;
		if (0==_stricmp(p_ext,".prj")	)	return true;
		if (0==_stricmp(p_ext,".lights")	)	return false;
	}
	if (0==_stricmp(p_name,"do_light") && 0==_stricmp(p_ext,".ltx"))	return true;
	...
	if (0==_stricmp(p_ext,".vcproj"))	return true;
	if (0==_stricmp(p_ext,".sln"))	return true;
	if (0==_stricmp(p_ext,".old"))	return true;
	if (0==_stricmp(p_ext,".rc"))	return true;
...
bool xrCompressor::IsFolderAccepted(CInifile& ltx, LPCSTR path, BOOL& recurse)
{
	...
	if( ltx.section_exist("exclude_folders") )
	{
		...
		for (auto ef_it=ef_sect.Data.begin(); ef_it!=ef_sect.Data.end(); ef_it++)
		{
