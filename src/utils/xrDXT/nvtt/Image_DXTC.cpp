...
Image_DXTC::Image_DXTC(): 
    m_pCompBytes(nullptr), m_pDecompBytes(nullptr)
{
}
...
bool Image_DXTC::LoadFromFile(LPCSTR filename)
{
    ...
    int next = 1;
    char fileupper[256];
    strncpy(fileupper, filename, sizeof(fileupper) - 1);
    ...
    if (m_CompFormat == PF_DXT1 || m_CompFormat == PF_DXT2 || m_CompFormat == PF_DXT3 || m_CompFormat == PF_DXT4 || m_CompFormat == PF_DXT5)
    {
...
inline void GetColorBlockColors(DXTColBlock* pBlock,
    Color8888* col_0, Color8888* col_1, Color8888* col_2, Color8888* col_3,  u16& wrd)
{
