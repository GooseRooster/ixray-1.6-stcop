...
{
	m_parent = p;
	const shared_str& hud_section = p->HudSection();
	m_fFlash_light_range = pSettings->r_float(hud_section, "flash_light_range");
	m_fOnOff_light_range = pSettings->r_float(hud_section, "onoff_light_range");
	Flash(false, 0.0f);
}
