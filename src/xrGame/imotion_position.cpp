...
#ifdef	DEBUG
	if( collide_obj )
		return make_string<const char*>("collide obj: %s", collide_obj->cName().c_str() );
	else
		return make_string<const char*>("collide static");
#else
	return std::string("");
#endif
}
...
static void save_blends( buffer_vector<sblend_save>& buffer, IKinematicsAnimated& KA )
{
	...
	struct scbl: public IterateBlendsCallback
	{
		...
		scbl(const scbl& other) = delete;
		scbl& operator=(const scbl& other) = delete;
...
float imotion_position::move( float dt, IKinematicsAnimated& KA )
{
	...
	for( u32 i = 0; i < iterations; ++i )
	{
		...
		if( dbg_imotion_collide_debug )
		{
			depth = 0;
			shell->CollideAll();
			interactive_motion_diagnostic( make_string<const char*>( " move (to ragdoll): deppth= %f", depth ) );
			DBG_OpenCashedDraw();
...
float imotion_position::motion_collide( float dt, IKinematicsAnimated& KA )
{
	...
	if( depth > depth_resolve )
	{
		...
		if( dbg_imotion_collide_debug )
		{
			//interactive_motion_diagnostic( make_string<const char*>( " motion_collide collided0: deppth= %f", depth ).c_str() );
		...
		if( dbg_imotion_collide_debug )
		{
			interactive_motion_diagnostic( make_string<const char*>( " motion_collide collided1: deppth= %f", depth ));
			DBG_OpenCashedDraw();
		...
		if( dbg_imotion_collide_debug )
		{
			interactive_motion_diagnostic( make_string<const char*>( " motion_collide collided2: deppth= %f", depth ) );
			DBG_OpenCashedDraw();
			shell->dbg_draw_geometry(0.02f, color_argb(255, 0, 255, 0));
			DBG_ClosedCashedDraw( 50000 );
		}
		...
		if( dbg_imotion_collide_debug )
		{
			depth = 0;
			shell->CollideAll();
			interactive_motion_diagnostic( make_string<const char*>(  " motion_collide restore: %f ", depth  ) );
			DBG_OpenCashedDraw();
