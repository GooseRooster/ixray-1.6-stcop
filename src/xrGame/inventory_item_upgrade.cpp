...
{
	for (const shared_str& it : m_upgrades)
	{
...
bool CInventoryItem::get_upgrades_str(string2048& res) const
{
	...
	for (const shared_str& it : m_upgrades)
	{
		upgr = Level().m_upgrade_manager->get_upgrade(it);
		if (upgr == nullptr)
		{
			continue;
		}
...
bool CInventoryItem::equal_upgrades(Upgrades_type const& other_upgrades) const
{
	...
	for (const shared_str& upgrade : m_upgrades)
	{
		bool upg_equal = false;

		for (const shared_str& other_upgrade : other_upgrades)
		{
			if (upgrade.equal(other_upgrade))
			{
				upg_equal = true;
...
void CInventoryItem::net_Spawn_install_upgrades(Upgrades_type saved_upgrades) // net_Spawn
{
	...
	for (const shared_str& it : saved_upgrades)
	{
		Level().m_upgrade_manager->upgrade_install(*this, it, true);
	}
}
