...
void CTrade::TransferItem(CInventoryItem* pItem, bool bBuying, bool bFree)
{
	...
	CGameObject* O1 = pPartner.inv_owner->cast_game_object();
	CGameObject* O2 = pThis.inv_owner->cast_game_object();
	...
	{
		if (CArtefact* pArtefact = pItem->cast_artefact())
		{
			CAI_Trader* pTrader = pThis.base->cast_trader();
...
u32	CTrade::GetItemPrice(PIItem pItem, bool b_buying, bool b_free)
{
	...
	CArtefact* pArtefact = pItem->cast_artefact();

	// computing base_cost
	float base_cost = 0.0f;
	if (pArtefact != nullptr && (pThis.type == TT_ACTOR) && (pPartner.type == TT_TRADER))
	{
		CAI_Trader* pTrader = pPartner.inv_owner->cast_trader();
		VERIFY(pTrader);
	...
	if (func)
	{
		result = iFloor(result * func(pThis.inv_owner->cast_game_object()->ID()));
	}
