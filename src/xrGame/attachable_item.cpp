...
void CAttachableItem::OnH_A_Chield()
{
	...
	const CInventoryOwner* inventory_owner = parent != nullptr ? parent->cast_inventory_owner() : nullptr;
	if (inventory_owner != nullptr && inventory_owner->attached(&item()))
	{
...
void CAttachableItem::enable(bool value)
{
	...
	{
		CObject* parent = object().H_Parent();
		if (CInventoryOwner* owner = parent != nullptr ? parent->cast_inventory_owner() : nullptr)
		{
			m_enabled = value;
			owner->attach(&item());
			object().setVisible(true);
