...
loophole::loophole	(luabind::object const &description) :
	...
	m_range			(0.f)
{
	VERIFY2			(luabind::get_type(description) == LUA_TTABLE, "invalid loophole description passed");
	...
	for (; I != E; ++I) {
		...
		const char* action_type = luabind::object_cast<const char*>(I.key());
...
{
	VERIFY2(luabind::get_type(transitions_table) == LUA_TTABLE, "invalid loophole description passed");
