...
//	}
	...
	if (object().memory().enemy().selected()) {
		CMemoryInfo					mem_object_ = object().memory().memory(object().memory().enemy().selected());

		if (mem_object_.m_object) {
			object().best_cover		(mem_object_.m_object_params.m_position);
		}
	}
}
