...
void w_bool(NET_Packet *self_, bool value)
{
	self_->w_u8		(value ? 1 : 0);
}

bool r_bool(NET_Packet *self_)
{
	return			(!!self_->r_u8());
}

ClientID r_clientID(NET_Packet *self_)
{
	ClientID		clientID;
...
template<> void class_exporter_lib<NET_Packet>::script_register(lua_State *L)
{
	module(L)
	[
